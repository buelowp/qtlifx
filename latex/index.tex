...
The headers are split between public and private, and only the public are installed. This is to avoid the potential of misusing the private A\+PI\textquotesingle{}s which don\textquotesingle{}t have a function outside of the manager class.
...
\begin{DoxyItemize}
\item The L\+I\+FX A\+PI is little endian (LE) or network byte order. This library currently assumes a LE C\+PU.
\item This library will not work on R\+Pi or BE C\+PU for the above assumption. There is a plan to fix for all endianness, just not today.
